...
Use explicit application use cases, thin controllers, server-authoritative validation and tests from `TESTING_STRATEGY.md`. Authentication remains blocked where `TECH_STACK_LOCK.md` requires a technical decision. Monetary representation is resolved by `TECH-ADR-004`: exact decimal calculation, PostgreSQL NUMERIC persistence and decimal-string API transport; currency, rounding and field precision/scale must never be invented. Entity identity is resolved by `TECH-ADR-003`: application-generated UUID v7, PostgreSQL native `uuid`, UUID-formatted API strings, and no Prisma/concrete UUID dependency in Domain code. The Backend test runner is resolved: `TECH-ADR-002` approves Vitest.
